...
\vspace{0.2cm}
\begin{tabular}{lll}
\textit{terme} ::= & | & \texttt{VARIABLE}  \\
& | & \texttt{PARLEFT} \ \textit{terme} \ \textit{terme} \ \texttt{PARRIGHT} \\
& |& \texttt{LAMBDA} \ \texttt{VARIABLE} \  . \ \textit{terme}
\end{tabular}
\vspace{0.2cm}
...
La compilation de ce fichier \texttt{lambdalexical.mll} va générer une fonction dont le nom
est celui de la règle (ici \texttt{token}).
Cette fonction prend comme argument le type \texttt{lexbuf} et rend le type \texttt{token}. 

\texttt{lexbuf}  est un type de données abstrait défini dans le module Lexing 
...
Nous définissons ici la grammaire du $\lambda$-calcul.
Nous retrouvons les constructeurs du type ML associés à chacune des règles de la grammaire.
Ces constructeurs seront préentés dans la section qui suit.
...
La compilation de ce fichier \texttt{lambdagrammar.mly} va générer une fonction dont le nom
est celui de l'axiome de notre grammaire (ici \texttt{exp}). Cette fonction prend deux arguments: la fonction de l'analyseur lexical qui génère les tokens et l'input. Elle rend le type
des expressions utilisées commes actions dans la grammaire.
\begin{Verbatim}
val exp :   (Lexing.lexbuf  -> token) -> Lexing.lexbuf -> Terme.terme
\end{Verbatim}

 Si le langage analysé n'est pas reconnu par la grammaire, l'exception \texttt{ParseError} est levée.
...
\vspace{0.5cm}
\begin{tabular}{lll}
\textit{exprule} & $::=$ & | \texttt{VARIABLE}  \\
					  &	    & | \texttt{PARLEFT} \  \textit{parrule} \\
					  & 		 & | \texttt{NEWLINE} \\
\textit{parrule} & $::=$ & | \texttt{LAMBDA} \ \textit{lambdarule}  \\
					  &       & | \textit{apprule} \\ 
\textit{lambdarule} & $::=$ & \texttt{VARIABLE}  \texttt{POINT}  \textit{exprule}  \texttt{PARRIGHT} \\
\textit{apprule} & $::=$    &  \textit{exprule} \textit{exprule}  \texttt{PARRIGHT} \\
\end{tabular}
\vspace{0.5cm}
...
Un terme du $\lambda $-calcul est donc un type ML composé, avec les constructeurs $Var$, $App$ et $Lam$.

Par exemple, le terme $ \lambda x.(x y) z $ est representé par la structure:



\texttt{App ((Lam ("x", (App ((Var "x"), (Var "y"))))), (Var "z"))}
...
Pour dessiner cet arbre, nous utilisons le très bon package \textsc{Tikz} qui permet facilement de représenter
...
On implémente deux fonctions \textsc{Ocaml}
qui permettent  d'afficher une expression de type $\lambda $-terme en code \LaTeX\ ou en code \textsc{Tikz}.


La fonction \texttt{varLibres} retourne les variables libres (ie. non liées) d'un $\lambda $-terme.
\begin{Verbatim}
...
Enfin, nous avons une fonction \texttt{fullReduc} qui permet d'itérer l'opération de
...
Nous avons le même résultat et les même étapes de réduction avec \texttt{betaNormal zm ;;}
...
\textsc{Agda}, qui ne possède pas un langage de tactiques permettant de construire le terme, 
doit expliciter le terme matérialisant la preuve. Cela peut se faire très élégamment
grâce à la belle syntaxe du langage.\cite[Program = Proof]{PP}
\begin{minted}{agda}
...
\textsc{Lean} se rapproche de \textsc{Coq} avec son langage de tactiques. En voici la preuve.
\begin{minted}{lean}
def surj {A B : Type} (f : A → B) : Prop :=
 (b : B) → ∃ a : A, f a = b

theorem cantor : ¬ (∃ f : ℕ → ℕ → Bool, surj f) := by
intro h
rcases h with ⟨f , SURJ⟩
let g := fun b => not b
let h := fun x => g (f x x)
rcases (SURJ h) with ⟨ x , B ⟩
let C : h x = f x x  := by simp [B]
simp [h] at C
simp [g] at C
\end{minted}
