\chapter{Le \texorpdfstring{$\lambda$}{lambda}-calcul simplement typé et les Pure Type Systems}
...
Pour présenter un système d'inférence de type, nous introduisons la constante de type \texttt{Int} à notre $\lambda$-calcul
simplement typé.
...
De même, nous enrichissons notre définition de terme avec le constructeur \texttt{Const of int} et la fonction binaire  \texttt{Plus} 
\begin{Verbatim}
type terme = 
  | Var of string 
  | App of terme * terme 
...
Nous utilisons pour l'affichage \textsc{Ocaml} les caractères UTF-8 :  \texttt{λ, π, →}
...
\begin{itemize}
  \item Le type \texttt{Nat} avec ses constructeurs \texttt{0} et \texttt{S}
  \item Le type de l'égalité \texttt{Eq} avec son unique constructeur \texttt{EqRefl}
  \item Le type \texttt{And} avec son unique constructeur \texttt{Conj} et ses fonctions
  \texttt{Proj1} et \texttt{Proj2}. L'affichage du type \texttt{And} se fera avec les caractères \verb+/+
  \item Le type \texttt{Or} avec ses constructeurs \texttt{OrIntrol} et \texttt{OrIntror} et sa fonction \texttt{Case}.
  L'affichage de ce type se fera avec les caractères \verb+|+
  \item Le type \texttt{False} sans constructeur, mais avec la fonction \texttt{FalseInd(t1,t2)} qui se
  réduit en \texttt{t1} si le type de \texttt{t2} est égal à \texttt{False}  (\textit{ex falso quodlibet})
\end{itemize}
...
Nous surchargeons notre terme algébrique de l'opérateur de point fixe \texttt{Y of terme} qui
se réduit en \texttt{Y t} \imp\ \texttt{t (Y t)}
\begin{minted}{ocaml}
let multF = 
