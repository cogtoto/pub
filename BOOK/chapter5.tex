...
Les symboles de constante \texttt{true}, \texttt{158}, les symboles de fonctions unaires \texttt{not}, \texttt{-},
les symboles de fonctions binaires \texttt{or}, etc. constituent la signature $\Sigma$ du langage.
...
Prenons par exemple le terme \texttt{Func("f", [Func("g", [Var "x"; Var "y"]); Var "z"])}
...
\texttt{string} vers \texttt{int} nécessaire à la manipulation des occurences $u$. 
...
Nous pouvons implémenter cela en OCAM avec le type abstrait \texttt{peano}
\begin{minted}{ocaml}
type peano = 
	| Zero
	| Succ of peano
...
\end{array}
$$
Ces quatre règles sont implémentées par la fonction \texttt{réduire} ci-dessous:
\begin{minted}{ocaml}
...
\textit{terme} que nous avons précedemment présenté, ainsi que la fonction de filtrage \texttt{filtre} et 
la fonction du substitution \texttt{substituer}.

Nous avons fait simple avec cette méthode naïve qui utilise les trois fonctions ci-dessous :
\begin{itemize}
	\item La première \texttt{rewrite} utilise la fonction \texttt{filtre} pour chercher une substitution
	...
	\item  La seconde \texttt{rewriteall} déroule l'ensemble des règles représentées par une liste de paires $(l,r)$ tant
	que la réécriture ne modifie par le terme.
	\item La troisième \texttt{rewrite-bourrin} itère la fonction précédente tant que l'on peut réduire le terme.
...
Avec une seule fonction parcourant le système d'équations, représentés en \textsc{Ocaml} par le type \texttt{(term * term) list},
...
On retrouve dans la fonction \texttt{unifier}, qui travaille uniquement sur une paire de terme, les différentes règles
de l'algorithme. La fonction \texttt{unifierliste} va unifier sa première paire en utilisant la substitution trouvée dans le reste
...
Définissons en \textsc{Prolog} le type des entiers naturels avec la fonction \texttt{nat}  d'arité 1, la fonction \texttt{s} d'arité 1 et la constante \texttt{0}.
d'arité 1 et la constante \texttt{0}.
...
On définit la fonction suivante : $diag(i) ≡$ si $a_i$ termine alors \texttt{boucler}, sinon \texttt{s'arrêter} \\
...
diag(p) = si halt (p) alors boucle()
          sinon stop() ;
\end{minted}
Quel est le comportement de \texttt{halt(diag(diag))} ?


S'il répond vrai, alors \texttt{diag} s'arrête alors qu'il est censé boucler par définition de \texttt{diag}


S'il répond faux, alors \texttt{diag} boucle, alors qu'il est censé s'arrêter par défintion de \texttt{diag}
