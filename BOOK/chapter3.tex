...
Un interprète est ainsi  une fonction \texttt{eval} telle que \texttt{(eval $\pi$ env)} \imp\  \texttt{valeur}
...
Voici la séquence du code, depuis le stream en entrée de l'analyseur lexical jusqu'à la sortie de l'évaluateur \texttt{eval}.
J'ai fait le choix d'avoir une représentation intermédiaire \texttt{ast} permettant de modéliser l'arbre syntaxique, et de faciliter
...
Et enfin une expression let \texttt{(let ((a 2) (b 3)) (plus a b))}
\begin{center}
...
en exprimer une valeur mod\'{e}lis\'{e}e avec le type \texttt{value}

La fonction \texttt{eval} est une fonction prenant pour  arguments une expression de type \texttt{ast} et un environnement.
Elle retourne une valeur de type \texttt{exp}. Voici sa signature: \\
\texttt{val eval : ast -> env -> exp = <fun>}
...
\texttt{Lambda (parametres, expression) -> Closure (parametres, expression, env)}     
...
En Scheme et Ocaml, nous pouvons voir dans l'exemple ci-dessous que l'évaluation de la définition de la
 lambda \texttt{inc} capture la valeur de \texttt{x} .
...
On remarquera également que le \texttt{letrec} de SCHEME peut être sémantiquement remplacé par un \texttt{let} associé de \texttt{set!}
...
\end{minted}
  ...
  Considérons l'expression \texttt{(letrec ((x1 e1) ... (xn en)) corps)}  qui, on le rappelle, est \'{e}quivalente à 
  \texttt{((lambda (x1 ... xn) corps) e1 ... en)}
...
\begin{minipage}{8cm}
    \textit{
      ...
      et le sujet du livre, si vous voulez, c’est précisément la lutte entre ce que lui offre la réalité et ce que,
       ...
       \vspace{1cm}
       Kαὶ εἶπεν ὁ θεὸς πρὸς Μωυσῆν Ἐγώ εἰμι ὁ ὤν.\\
...
En rendant explicite la procédure \texttt{eval} et ses acolytes \texttt{evlis, invoke, ...} dans l'environnement
\texttt{env}, la procédure \texttt{evaluate} pourra être évaluée par elle-même. Le premier argument évalué de la fonction
est le symbole \texttt{env}. Ce symbole devra être contenu dans l'environnement \texttt{env}, c'est-à-dire dans lui-même\dots

Nous avons \texttt{(eval '(eval '}$\pi$ \texttt{env) env)} $\equiv$ 
 \texttt{(evaluate '$\pi$ env)} 
...
L'environnement doit contenir la valeur du symbole \texttt{env}. Il doit faire référence à lui-même.
...
On peut citer l'exemple de la fonction \texttt{eval} rendant accessible dans Scheme le process d'évaluation. 
\texttt{(eval '$\pi$ env)} = $\pi$

L'autre exemple que nous implémenterons est la réification de la continuation courante, mis à disposition
par la fonction \texttt{call/cc}. Cette fonction prend en argument une lambda avec un seul paramètre qui récupère la
continuation courante de l'expression en cours d'évaluation.
\begin{minted}{scheme}
E = (e1 e2 ... (call/cc (lambda (k) ei)) ... en)	
\end{minted}
  Si \texttt{ei} ne fait pas appel à \texttt{k}, alors \texttt{ei} est évaluée normalement, ainsi que \texttt{E}. 
Dans le cas contraire, \texttt{k} est appelée, liée à la continuation courante. Le résultat de \texttt{ei} est ainsi rendue
à cette continuation capturée \texttt{(e1 ... [] ... en)}. Autrement écrit \texttt{k = (lambda(v) (e1 ... v ... en))}
\begin{minted}{scheme}
(+ 5 (call/cc (lambda (k) (* 2 (k 8)))))  = 12
\end{minted} 


L'environnement \texttt{global-env} partagé avec les différents interprètes $\mathcal{I}_i$ de notre tour de babel est aussi considéré
...
Comme exemple, citons la fonction \texttt{quote} qui n'évalue pas son argument et le rend tel quel. \texttt{quote} est
...
réification & program vers data &  \texttt{((reifier-to-cloture proc) (cdr e) r k)}   \\ \hline
réflection  & data vers program & \texttt{(cloture-to-reifier (lambda (e r k) exp))} \\ \hline
\end{tabular}
...
\begin{center}
\texttt{(eval $\pi\ \rho\ \kappa$)} \imp\  \texttt{valeur}
\end{center}

Nous reprenons ici le code de l'excellent article \textit{a Simple Reflective Interpreter} \cite{reflective}
La fonction \texttt{evaluate} implémente un interprète Lisp en mode CPS de manière très naturelle.
...
\begin{enumerate}
  \item Les fonctions utilisateurs \texttt{(cloture (parl) exp env)}
  \item Les fonctions réifiées \texttt{(reifier (e r k ) exp )}
  \item Les fonctions primitives \texttt{(primitive nom )}
\end{enumerate}
...
Une fonction réifiée prend trois paramètres \texttt{e}, \texttt{r} et \texttt{k}.
\begin{itemize}
  \item  \texttt{e} est lié à la liste des arguments non évalués de l'application.
  \item \texttt{r} est lié à l'environnement de l'interprète évaluant l'application.
  \item \texttt{k} est lié à la continuation de l'interprète évaluant l'application.
\end{itemize}
...
Les fonctions \texttt{callcc} et \texttt{quote} seront très simplement codées de la manière suivante:
\begin{minted}{scheme}
(define callcc (cloture-to-reifier (lambda (e r k) ((evaluate (car e) r id) k))))
(define quote (cloture-to-reifier (lambda (e r k) (k (car e) ))))
\end{minted}

Dès le niveau 1 de notre tour, c'est-à-dire lorsque la fonction \texttt{evaluate} n'est plus évaluée par Scheme 
mais par elle-même, les formes spéciales \texttt{if, quote, begin, define} sont représentées par des fonctions réifiées.
...
\begin{minipage}{10.5cm}
  La fonction \texttt{openloop} peut être lancée à volonté et peut créer une succession 
  ...
  $n+1$ n'apporte rien par rapport à l'étage $n$. L'application \texttt{eval} est idempotente car 
  $\forall \pi$ \texttt{(eval '}$\pi$\texttt{)} $=$ \texttt{(eval (eval '}$\pi$\texttt{))}.
  Autrement dit, la valeur \texttt{(eval '}$\pi$\texttt{)} est un point fixe de \texttt{eval}.
