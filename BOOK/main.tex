...
\title{Le calcul par la réduction et la résolution \\
	 ...
	 Exemples en \textsc{Scheme}, \textsc{Ocaml}, \textsc{Coq}, \textsc{Agda} et \textsc{Lean}\\ \vspace{1cm}
   ᾿Αγεωμέτρητος μηδε εἰσίτω \\
      }
...
\chapter{Ἐν ἀρχή, Gn 1.1 et Jn 1.1}
...
Le type faux est un type inhabité, donc nous le définissons sans constructeur : \texttt{type faux = | }

Nous pouvons par exemple prouver le théorème du modus tollens : \\

\texttt{let ModusTollens (hfq:'q->faux) (hpq:'p->'q) (hp:'p) = hfq (hpq hp)}  \\
Le type de cette expression est  \texttt{('q -> faux) -> ('p -> 'q) -> 'p -> faux}  \\
...
\textsc{Coq}, \textsc{Agda} et \textsc{Lean} implémentent ce formalisme.
