...
\texttt{(état-suivant, caractère-écrit, direction)}
...
L'état \texttt{q0} va rechercher une parenthèse \texttt{>} en allant vers la droite.

L'état \texttt{q1} va rechercher une parenthèse \texttt{<} en allant vers la gauche.

L'état \texttt{q2} va rechercher une parenthèse \texttt{>} en allant vers la gauche.


Les parenthèses matchées sont remplacées par le caractère \texttt{X}.
Le passage à  l'état final \texttt{qf} est accompagné par l'écriture \texttt{oui} ou
\texttt{non} sur la bande suivant si l'expression est ou non correctement parenthésée.
...
\end{Verbatim}
\end{tiny}
\subsection{Sudoku - un algorithme simple et récursif en \textsc{Lean}}
Voici un algorithme en \textsc{Lean}. 
La grille du Sudoku est représenté par une fonction de \texttt{Coord → Option Digit}.
\begin{minted}{lean4}
import Mathlib
open Nat
...
def Coord := Fin 9 × Fin 9

instance : DecidableEq Coord :=
  inferInstanceAs (DecidableEq (Fin 9 × Fin 9))

def Grid := Coord → Option Digit

def sameRow (a b : Coord) : Bool := a.1 == b.1
def sameCol (a b : Coord) : Bool := a.2 == b.2

def sameBlock (a b : Coord) : Bool :=
  (a.1.val / 3 == b.1.val / 3) &&
  (a.2.val / 3 == b.2.val / 3)

def allCoords : List Coord := List.product (List.finRange 9) (List.finRange 9)

def conflicts (g : Grid) (c : Coord) (d : Digit) : Bool :=
  allCoords.any (fun c' => (c' != c) &&
    (sameRow c c' || sameCol c c' || sameBlock c c') &&
    match g c' with
    | some d' => d' == d
    | none    => false)

def candidates (g : Grid) (c : Coord) : List Digit :=
  (List.finRange 9).filter (fun d => !(conflicts g c d))

def set (g : Grid) (c : Coord) (d : Digit) : Grid :=
  fun c' => if c' == c then some d else g c'

def emptyCells (g : Grid) : List Coord :=
  allCoords.filter (fun c => g c == none)

partial def solve : Grid → Option Grid
| g =>
  match emptyCells g with
  | []      => some g
  | c :: _  =>
      (candidates g c).findSome? (fun d => solve (set g c d))
